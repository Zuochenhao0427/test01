...
kmodel = pd.read_csv('model_data180630.csv')
kmodel.head(5)
## 基金数据导入
...
####### Cahart四因子OLS回归模型中新加入了rbrf - 债券因子 #########
...
def res_caculator(f):
    ...
    # 记录回归方程中各参数取值
    coef = {}
    coef.update(alpha_dot = result.params[0], mkt_b = result.params[1], smb_b = result.params[2], hml_b = result.params[3],  umd_b = result.params[4]，rbrf_b = result.params[5])
...
def k_fake_alpha_calculator(res, coef, cahart_factors):
    # 通过残差抽样计算伪回报率
    fake_y = cahart_factors['mkt_rf']*coef.get('mkt_b') + cahart_factors['smb']*coef.get('smb_b') + cahart_factors['hml']*coef.get('hml_b') + cahart_factors['umd']*coef.get('umd_b') + cahart_factors['rbrf']*coef.get('rbrf_b') + res
...
funds1 = kmodel[kmodel['fund_id'] == funds_list[0]].reset_index(drop=True) # 使用基金清单中第一支基金测试以上残差计算函数
...
Act = series_manipulator(t_alpha,t_ta)[1].flatten() # EXP: Act.shape = (基金总数,)
...
Mean_Sim = np.array(sorted_ta).mean(axis=0).flatten() # EXP: shape = (基金总数,)
...
k_ta_dist = k_ta_array.T # EXP: f_ta_dist.shape = (基金总数,1000)
...
print(len(perc_act), perc_act[:10]) # EXP: len(perc_act) = len(funds_list) = 基金总数, 且从小到大有序排列
